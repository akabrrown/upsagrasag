...
export default function AdminFocusAreasPage() {
  const { data: records, isLoading, mutate } = useSWR<FocusArea[]>('/api/admin/focus-areas', fetcher);
  
  const [view, setView] = useState<ViewState>('list');
  const [selectedRecord, setSelectedRecord] = useState<FocusArea | null>(null);

  const { register, handleSubmit, reset, setValue, control, formState: { errors, isSubmitting } } = useForm<FocusArea>({
    resolver: zodResolver(focusAreaSchema) as any,
    defaultValues: { title: '', description: '', image_url: '', icon_name: '', display_order: 0 }
  });
  ...
  const handleOpenAdd = () => {
    reset({ title: '', description: '', image_url: '', icon_name: '', display_order: 0 });
    setSelectedRecord(null);
    setView('add');
  };

  const handleOpenEdit = (raw_item: FocusArea) => {
  ...
  const handleOpenDetails = (item: FocusArea) => {
    setSelectedRecord(item);
    setView('details');
  };

  const handleDelete = async (item: FocusArea) => {
    if(!confirm(`Are you sure you want to remove "${item.title}"?`)) return;
    try {
      const res = await fetch(`/api/admin/focus-areas/${item.id}`, { method: 'DELETE' });
  ...
  const onSubmit = async (data: FocusArea) => {
    try {
      const isEditing = view === 'edit' && selectedRecord;
      const url = isEditing ? `/api/admin/focus-areas/${selectedRecord.id}` : '/api/admin/focus-areas';
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Dashboard &gt; Focus Areas</p>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="border-b border-gray-100 text-xs font-semibold text-gray-500 uppercase tracking-wider bg-gray-50/50">
                ...
                <th className="px-6 py-4 font-medium">Icon / Image</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {isLoading ? (
                ...
                <tr><td colSpan={4} className="px-6 py-8 text-center text-gray-500">No focus areas found.</td></tr>
              ) : (
                  ...
                  <tr key={record.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 text-sm text-gray-600">
                        ...
                        <span className="font-medium text-gray-900">{record.icon_name || 'No Icon'}</span>
                      </div>
                    </td>
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
      <div className="flex items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">{view === 'edit' ? 'Edit Focus Area' : 'Add New Focus Area'}</h1>
            <p className="text-xs text-gray-500 mt-0.5">Dashboard &gt; Focus Areas &gt; {view === 'edit' ? 'Edit' : 'Add New'}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      <form className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-5">
            <h2 className="font-semibold text-gray-900 text-lg">Image (Optional)</h2>
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
        <div className="flex items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Focus Area Details</h1>
            </div>
          </div>
        </div>
