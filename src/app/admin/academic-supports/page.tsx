...
import { academicSupportSchema, AcademicSupport } from '@/types/admin';
import { 
...
export default function AdminAcademicSupportsPage() {
  const { data: records, isLoading, mutate } = useSWR<AcademicSupport[]>('/api/admin/academic_supports', fetcher);
  
  const [view, setView] = useState<ViewState>('list');
  const [selectedRecord, setSelectedRecord] = useState<AcademicSupport | null>(null);

  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<AcademicSupport>({
    resolver: zodResolver(academicSupportSchema) as any,
  ...
  const handleOpenDetails = (item: AcademicSupport) => {
    setSelectedRecord(item);
    setView('details');
  };

  const handleDelete = async (item: AcademicSupport) => {
    if(!confirm(`Are you sure you want to remove "${item.title}"?`)) return;
    try {
      const res = await fetch(`/api/admin/academic_supports/${item.id}`, { method: 'DELETE' });
  ...
  const onSubmit = async (data: AcademicSupport) => {
    try {
      const isEditing = view === 'edit' && selectedRecord;
      const url = isEditing ? `/api/admin/academic_supports/${selectedRecord.id}` : '/api/admin/academic_supports';
  ...
  const ListView = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Academic Supports</h1>
          <p className="text-sm text-gray-500 mt-1">Dashboard &gt; Academic Supports</p>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            ...
            <tbody className="divide-y divide-gray-50">
              {isLoading ? (
                ...
                <tr><td colSpan={4} className="px-6 py-8 text-center text-gray-500">No supports found.</td></tr>
              ) : (
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
      <div className="flex items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">{view === 'edit' ? 'Edit Support' : 'Add New Support'}</h1>
            <p className="text-xs text-gray-500 mt-0.5">Dashboard &gt; Academic Supports &gt; {view === 'edit' ? 'Edit' : 'Add New'}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
                ...
                <input 
                  {...register('icon')} 
                  placeholder="e.g. BookOpen, Layers, CheckSquare"
                  className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#2563eb]/20 focus:border-[#2563eb] transition-all text-sm"
                />
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
        <div className="flex items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Support Details</h1>
            </div>
          </div>
        </div>
